class ConfigureAppJob < ApplicationJob
  ...
  def perform(app, params, user)
    ...
    begin
      self.send("perform_#{platform}", app, params, user)
  ...
  private
  def perform_android(app, params, user)
    ...
    tag_name = "#{app.internal_slug}-#{app.android_config['version_name']}"

    open_repo repo

    logger.tagged('Android') do
      manipulate_repo repo, app, user, tag_name do |_git|
  ...
  def perform_ios(app, params, user)
    ...
    tag_name = "#{app.internal_slug}-#{app.ios_config['version_name']}"
  ...
  def perform_both(app, params, user)
    perform_android app, params, user
    perform_ios app, params, user
  end
end
